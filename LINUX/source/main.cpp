...
static char map[784];
GLint level = 1; // Can either be level 1, level 2							 
...
/* Turning this down makes it easier for pacman to fit through hallways */
...
void idle ()
{
    ...
    /**********************************************/
    /* CHANGE LEVEL WHEN ALL THE PELLETS ARE GONE */
    /**********************************************/
	if (pelletTiles.empty() && powerPelletTiles.empty() && level == 1)
	{
        if (level == 1)
        {    
            reset(2);
	...
	if((ghost1->x<(pacman->x+1)) && (ghost1->y<(pacman->y+1)) && (ghost1->z<(pacman->z+1)))
	{
		if((ghost1->x>(pacman->x-1)) && (ghost1->y>(pacman->y-1)) && (ghost1->z>(pacman->z-1))){
			if(pacman->frenzy)
            {
				ghost1->initPosition(17.0f, 0.2f, 11.0f);
				g_dead= 1;
			}
			if(!pacman->frenzy)
            {
...
void help()
{
    ...
    << "Directional Arrows: move Pacman" << endl
    << "F10: reset view in case your view parameters get out of control" << endl
    << "W : Forward" << endl
    << "S : Backward" << endl
    << "A : Strafe Left" << endl
    << "D : Strafe Right" << endl
    << "= : Zoom In with Motion" << endl
    << "- : Zoom Out with Motion" << endl
    << "+ : Zoom In with Fovy" << endl 
    << "_ : Zoom Out with Fovy" << endl 
    << "[ and ] for roll" << endl
    << ", and . for orbit" << endl
    ...
    << "F12: Turn OFF all the Lamps" << endl
    << "1: Pacman cam" << endl
    << "2, 3, 4, 5: Ghost1, Ghost2, Ghost3, Ghost4 Cam" << endl
    << "6, 7, 8, 9: Light1, Light2, Light3, Light4 Cam" << endl
    << "0: Initial Cam" << endl
    << "P: pause" << endl
    << "B: Pacman Outfit Toggling" <<endl
    << "C: Toggle Textures" <<endl
    << "X: Pellet Texture Toggling" << endl
    << "Z: Shading Toggling" << endl
    << "H: Help" << endl
    << endl;
}
	...
	else
    {
		switch (key)
		{
			...
			case 'd':
				...
				break;
            case 'i':
                currentCamera->pitch(rotStep);
                break;
            case 'k':
                currentCamera->pitch(-rotStep);
                break;
            case 'j':
                currentCamera->yaw(rotStep);
                break;
            case 'l':
                currentCamera->yaw(-rotStep);
                break;
...
void setupLighting()
{
    /* Set the ambient, diffuse and specular of the lamps */
...
void initCameras()
{
    ...
    lamp1Cam.setCenter(lamp1->x + 5, lamp1->y - 4, lamp1->z + 5);
...
void loadMap(GLuint load)
{
    ...
    /* Clear all the level information */
	walls.clear();
	pelletTiles.clear();
